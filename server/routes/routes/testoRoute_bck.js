...
module.exports = (app, db) => {
  ...
  async function getUserTestRank(params){
    ...
    rankQuery +=" WHERE testId = '" + params.testId + "' AND packageId = '"+ params.packageId +"'";

    let aqlQuery = "SELECT userId, score as score, FIND_IN_SET( score, ("+rankQuery+")) AS rank FROM student_result ";
    aqlQuery +=" WHERE testId = '" + params.testId + "' AND packageId = '" + params.packageId +"' AND userId = '"+params.userId + "'";
  ...
  async function testChapterReport(params){
    ...
    var chpresult =[];
    var chpIdArr = [];
    console.log("testChapterReport -----", params)
    ...
    console.log("testChapterReport result == myyyyyy ----", chapResult);
    return chapResult;
  }
  ...
  app.post('/report-user-stand', (req, res)=>{
    if(req.body){
      ...
      if(params.userId){
        ...
        //selectColumnsconsole.log("my analysis", params)
        //let whereObj ={where: {"userId":params.userId, "testId":params.testId, "packageId":params.packageId}};
        ...
        testAttempted.findAll(findObj).then(async(attemptResult) => {
          ...
          for(var marks = 0; marks<100; marks = marks+5 ){
            ...
            studens_marks.push({marks_range, userCount});
            
          }
          res.status(200).json({userMarks, studens_marks });
        });
      }
    }
  });
